 ...
 *
 * Requirements:
 *  - `deno task compile` or `deno compile` must produce `bin/modmux`
...
Deno.test({
  ...
  async fn() {
    ...
    const stop = await new Deno.Command("bin/modmux", {
      args: ["stop"],
      stdout: "piped",
    }).output();
    const stopText = new TextDecoder().decode(stop.stdout).trim();
    assertEquals(stopText, "Modmux stopped.");
...
Deno.test({
  name: "daemon — modmux start prints 'already running' when running",
  ignore: true, // TODO: enable after T014/T016/T017 complete
  async fn() {
    const proc = new Deno.Command("bin/modmux", {
      args: ["start"],
...
Deno.test({
  name: "daemon — modmux stop when not running prints 'not running'",
  ignore: true, // TODO: enable after T014/T016/T017 complete
  async fn() {
    const out = await new Deno.Command("bin/modmux", {
      args: ["stop"],
      stdout: "piped",
    }).output();
    const text = new TextDecoder().decode(out.stdout).trim();
    assertEquals(text, "Modmux is not running.");
  },
});
